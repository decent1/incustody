...
import DeleteMatterModal from '../../../components/Modal/DeleteMatterModal'
...
export default function MatterHistory({
  ...
  async function getData(){
    const response = await request.GetRequestWithAuthentication(api.GetMatterDetailsAPI(route.params.item.id));
    if(response.success){
  ...
  const ChangeStatus = async () => {
    ...
    if(response.success){
      Helper.showToast('Matter Published Successfully')
      getData()
    }
  }
      ...
      <View style={commonStyles.container}>
        ...
        <ScrollView>
          ...
          <Modal
            animationType="slide"
            transparent={true}
            visible={modalVisible_Agents}
            onRequestClose={() => { setModalVisible_Agents(false); }}>
            <AgentsModal 
              item={item.agent_requests}
              modalVisible={() => { setModalVisible_Agents(false) }}
              matter_id={item.id}
              onSuccess={() => { getData() }}
            />
          </Modal>
          <Modal
             ...
             onRequestClose={() => { setModalVisible_Delete(false); }}> 
          ...
          <View style={styles.formDetails}>
            <View style={styles.formHeaderContainer}>
              ...
              <TouchableOpacity
                  activeOpacity={0.8}
                  onPress={() => {
                    navigation.navigate('UpdateMatter',{
                      item:item
                    })
                  }} 
                  style={[
                ...
                <TouchableOpacity
                  ...
                  <LinearGradient
                    ...
                    style={styles.publishNowButton}>
                    {
                      ...
                      : <Text style={styles.publishNowButtonText}>Publish Now</Text>
                    }
                  </LinearGradient>
                </TouchableOpacity>
                :
          ...
          </View>
          {
            ...
            ?
              <View style={styles.agenstDetailContainer}>
              <View style={{ flex:1, }}>
                <Text style={{ fontFamily:Fonts.semiBold, color:Colors.primary}}>
                  {item.agent_requests.length == 0 ? 'No Agent Requested Yet' :  `Requsted by (${item.agent_requests.length}) Agent(s)`}
                </Text>
              </View>
              {
                item.agent_requests.length == 0
                ?
                  null
                :
                  <TouchableOpacity 
                    onPress={() => setModalVisible_Agents(true)}
                    style={styles.viewAllAgentsButton}>
                    <Text style={styles.viewAllAgentsButtonText}>
                      {'View All Agents'}
                    </Text>
                  </TouchableOpacity>
              }
            ...
            <View style={styles.agenstDetailContainer}>
              <View style={{ flex:1, }}>
                <Text style={{ fontFamily:Fonts.semiBold, color:Colors.primary}}>
                  {`Matter Approved to ${item.assigned_agent.first_name} ${item.assigned_agent.last_name}`}
                </Text>
                {
                  item.status == 'completed-with-invoice' || item.status == 'completed-without-invoice'
                  ?
                  <Text style={{ 
