...
export default function UpdateMatter({navigation, route}) {
  const item = route.params.item;  
  const user = useSelector(state => state.user);

  const [defender_name, setDefender_name] = useState(item?.defender_name) //done
  const [office_incharge, setOffice_incharge] = useState(item?.office_incharge) //done

  const [offence_id, setOffence_id] = useState(item?.offence?.id) //done
  const [station_id, setStation_id] = useState(item?.station?.id) //done
  const [court_id, setCourt_id] = useState(item?.court?.id) //done
  const [assigned_agent_id, setAssigned_agent_id] = useState(item?.assigned_agent_id) //done

  const [user_id, setUser_id] = useState(user.id) //done
  const [status, setStatus] = useState('') 
  const [contact, setContact] = useState(item?.contact) //done
  const [note, setNote] = useState(item?.note); //done
  const [attachment, setAttachment] = useState(item?.attachment) //done
  const [selectedFile, setSelectedFile] = useState('')

  const [selectedOffence, setSelectedOffence] = useState(item?.offence) //done
  const [modalVisibleOffence, setModalVisibleOffence] = useState(false); 

  const [selectedStation, setSelectedStation] = useState(item?.station) //done
  const [modalVisibleStation, setModalVisibleStation] = useState(false);

  const [selectedCourt, setSelectedCourt] = useState(item?.court)  //done
  const [modalVisibleCourt, setModalVisibleCourt] = useState(false);

  const [selectedAgent, setSelectedAgent] = useState(item?.assigned_agent) //done
  ...
  const [matterChoice, setMatterChoice] = useState(item?.court_id != null ? 2 : 1);
  ...
  const addMatter = async () => {
    ...
    if(response.success){
      ...
      Alert.alert('Success', 'Matter Updated successfully')
      navigation.goBack()
    }
    else{
    ...
    <Drawer ref={_drawer}
      ...
      <TouchableOpacity onPress={() => {
        ...
        <ScrollView>
          ...
          <Text style={styles.des}>{'Solicitor/ Update matter'}</Text>
          <View style={styles.formDetails}>
            ...
            <TouchableOpacity
              ...
              <View style={{ flex: 1, justifyContent: 'center', }}>
                <Text style={styles.accountTypeText}>{selectedOffence == '' || selectedOffence == undefined  ? 'Select Offence' : selectedOffence.name}</Text>
              </View>
              ...
              {/* Station */}
            {
              ...
              <TouchableOpacity
                ...
                <View style={{ flex: 1, justifyContent: 'center', }}>
                  <Text style={styles.accountTypeText}>{selectedStation == '' || selectedStation == undefined ? 'Select Station' : selectedStation.name}</Text>
                </View>
                <Entypo name={'chevron-down'} size={20} color={'#798593'} />
              </TouchableOpacity>
            ...
            :
              <View>
                ...
                style={styles.dropDownInput}>
                <View style={{ flex: 1, justifyContent: 'center', }}>
                  <Text style={styles.accountTypeText}>{selectedCourt == '' || selectedCourt == undefined ? 'Select Court' : selectedCourt.name}</Text>
                </View>
                <Entypo name={'chevron-down'} size={20} color={'#798593'} />
              </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              <View style={styles.accountTypeChildContainer}>
                <Text style={styles.accountTypeText}>
                  {contact == '' || contact == undefined ? 'Select Contact Type' : contact}
                </Text>
              </View>
              <Entypo name={'chevron-down'} size={20} color={'#798593'} />
            </TouchableOpacity>
            ...
            <TouchableOpacity
                ...
                style={styles.dropDownInput}>
                <View style={{ flex: 1, justifyContent: 'center', }}>
                  <Text style={styles.accountTypeText}>{selectedAgent == '' || selectedAgent == undefined ? 'Select Agent' : `${selectedAgent.first_name} ${selectedAgent.last_name}`}</Text>
                </View>
                <Entypo name={'chevron-down'} size={20} color={'#798593'} />
                {
