...
export default function ListItemCourt({
    item,
    navigation,
    onSuccess
}){
    ...
    style={styles.container}>
        <View style={styles.rowContainer}>
            <View style={styles.itemContainer}>
                <Text style={styles.des}>{'Court'}</Text>
        ...
        <View style={[styles.rowContainer,{ marginTop:20 }]}>
            <View style={styles.itemContainer}>
                <Text style={styles.des}>{'Phone'}</Text>
        ...
        <View style={styles.actionButtonContainer}>
              <TouchableOpacity
                  activeOpacity={0.8}
                  onPress={() => {
                    navigation.navigate('UpdateCourt',{
                      item:item
                    })
                  }} 
                  style={[
                    ...
                    Alert.alert(
                      ...
                      [
                        ...
                        {text: 'OK', onPress: async () => {
                          const response = await request.DeleteRequestWithAuthentication(api.DeleteCourtAPI(item.id))
                          if(response.success){
                            Helper.showToast('Court Deleted Successfully')
                            onSuccess(item.id)
                          }
                          else{
